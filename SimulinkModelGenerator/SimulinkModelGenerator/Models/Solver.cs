...
namespace SimulinkModelGenerator.Models
{
    internal class Solver
    {
        internal List<Parameter> Parameters =>
            new List<Parameter>()
            {
                ...
                new Parameter() { Name = "ZeroCrossAlgorithm", Text = AdditionalSolverOptions.ZeroCrossingAlgorithm.GetDescription() },
                new Parameter() { Name = "ZeroCrossControl", Text = AdditionalSolverOptions.ZeroCrossingControl.GetDescription() }
            };
    ...
    internal class AdditionalSolverOptions
    {
        ...
        {
            if (initialStep < 0 || minStep < 0 || maxStep < 0)
                throw new SimulinkModelGeneratorException("Step size numbers must be positive or left empty");

            if (initialStep < minStep)
                throw new SimulinkModelGeneratorException($"Initial step size ({initialStep}) is smaller than minimum step size ({minStep}).");

            if (initialStep > (maxStep ?? 0.28))
                throw new SimulinkModelGeneratorException($"Initial step size ({initialStep}) is greater than maximum step size ({maxStep ?? 0.28}).");

            if (minStep >= (maxStep ?? 0.28))
                throw new SimulinkModelGeneratorException($"Maximum step size ({maxStep ?? 0.28}) must be greater than minimum step size ({minStep}).");
        ...
        internal void SetTolerance(double? relativeTolerance = null, double? absoluteTolerance = null)
        {
            if (relativeTolerance < 0 || absoluteTolerance < 0)
                throw new SimulinkModelGeneratorException("Tolerance numbers must be positive or left empty");
